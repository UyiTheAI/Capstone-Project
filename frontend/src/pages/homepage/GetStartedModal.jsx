...
export default function GetStartedModal({ onClose, onAlreadyHaveAccount, onProceedToPayment }) {
  ...
  const handleContinue = () => {
    if (!firstName || !lastName || !email || !password) { setError(t("errAllFields")); return; }
  ...
  const STEPS = [
    { n:"1", label:t("stepYourInfo"), active:true },
    { n:"2", label:t("stepPayment"),  active:false },
    { n:"3", label:t("stepDone"),     active:false },
  ];
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.75)", zIndex:3000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:"#fff", borderRadius:24, width:"100%", maxWidth:480, overflow:"hidden", fontFamily:"var(--font-body)", boxShadow:"0 24px 80px rgba(0,0,0,.3)" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#f5b800,#ffdd57)", padding:"24px 32px", position:"relative" }}>
          ...
          <h2 style={{ fontFamily:"var(--font-head)", fontSize:32, color:"#1a1a1a", margin:"0 0 4px", lineHeight:1 }}>{t("modalTitle")}</h2>
        </div>
        ...
        <div style={{ padding:"28px 32px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, fontWeight:700, color:"#555", marginBottom:6, display:"block" }}>{t("firstName").toUpperCase()}</label>
              <input style={inputStyle} value={firstName} onChange={e=>setFirstName(e.target.value)} placeholder="John" />
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, fontWeight:700, color:"#555", marginBottom:6, display:"block" }}>{t("lastName").toUpperCase()}</label>
              <input style={inputStyle} value={lastName} onChange={e=>setLastName(e.target.value)} placeholder="Smith" />
            </div>
          </div>

          <div style={{ marginBottom:12 }}>
            <label style={{ fontSize:12, fontWeight:700, color:"#555", marginBottom:6, display:"block" }}>{t("email").toUpperCase()}</label>
            <input style={inputStyle} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@restaurant.com" />
          </div>

          <div style={{ marginBottom:20 }}>
            <label style={{ fontSize:12, fontWeight:700, color:"#555", marginBottom:6, display:"block" }}>{t("password").toUpperCase()}</label>
            <input style={inputStyle} type="password" value={password} onChange={e=>setPassword(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&handleContinue()} placeholder={t("errPasswordShort")} />
          </div>
          ...
          <p style={{ fontSize:13, textAlign:"center", color:"#888", margin:0 }}>
            {t("alreadyAccount")}{" "}
            <span onClick={onAlreadyHaveAccount} style={{ color:"#f5b800", cursor:"pointer", fontWeight:700 }}>{t("signIn")}</span>
          </p>
        </div>
      </div>
    </div>
