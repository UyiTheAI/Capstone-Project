...
function ShiftModal({ emp, date, shift, onSave, onDelete, onClose }) {
  ...
  const [role,   setRole]   = useState(isEdit ? (shift.role || "Waitstaff") : "Waitstaff");
      ...
      <div style={{ background:"#fff", borderRadius:20, width:420, maxWidth:"95vw",
        ...
        <div style={{ background:"#1a1a1a", padding:"18px 24px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ color:"#f5b800", fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:1 }}>
              {isEdit ? (t("editShift")||"Edit Shift") : (t("addShift")||"Add Shift")}
            </div>
        ...
        <div style={{ padding:"20px 24px" }}>
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>Quick Select</div>
          ...
          <div style={{ display:"flex", gap:12, marginBottom:16 }}>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:1, marginBottom:6 }}>Start</div>
              <input type="time" value={start} onChange={e=>{setStart(e.target.value); setPreset("Custom");}}
                style={{ width:"100%", padding:"9px 12px", border:"1.5px solid #e0e0e0", borderRadius:10, fontSize:14, fontWeight:600, boxSizing:"border-box" }} />
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:1, marginBottom:6 }}>End</div>
          ...
          {duration > 0 && (
            <div style={{ textAlign:"center", marginBottom:14 }}>
              <span style={{ background:"#f0f9ff", color:"#0891b2", fontSize:12, fontWeight:700, padding:"4px 14px", borderRadius:20 }}>
          ...
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>Role</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {ROLES.map(r => {
                const c = rc(r);
                return (
          ...
          <div style={{ marginBottom:20 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>Area (optional)</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {AREAS.map(a => (
          ...
          <div style={{ display:"flex", gap:10 }}>
            {isEdit && (
              <button onClick={() => onDelete(shift._id)}
                style={{ padding:"10px 18px", border:"1.5px solid #fee2e2", background:"#fff", color:"#dc2626", borderRadius:10, cursor:"pointer", fontWeight:700, fontSize:13 }}>
                🗑 Delete
              </button>
            )}
            <button onClick={onClose}
              style={{ flex:1, padding:"10px 0", border:"1.5px solid #e0e0e0", background:"#fff", color:"#666", borderRadius:10, cursor:"pointer", fontWeight:700, fontSize:13 }}>
              Cancel
            </button>
            <button onClick={handleSave} disabled={saving || !start || !end || !role}
              style={{ flex:2, padding:"10px 0", border:"none", background:"#1a1a1a", color:"#f5b800", borderRadius:10, cursor:"pointer", fontWeight:800, fontSize:13, opacity:(!start||!end||!role)?0.5:1 }}>
              {saving ? "Saving..." : isEdit ? (t("updateShift")||"Update Shift") : (t("addShift")||"Add Shift")}
            </button>
          </div>
        </div>
      </div>
...
export default function ManagerSchedule({ user }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setEmployees(empRes.data.employees || empRes.data.users || []);
      setShifts(shiftRes.data.shifts || []);
    } catch(e) {
      console.error("Schedule load error:", e.message);
    } finally {
      setLoading(false);
    }
  }, [weekOffset]);
  ...
  const handleSaveShift = async ({ startTime, endTime, role, area, date, employeeId }) => {
    try {
      ...
      showToast(t("shiftAdded")||"Shift added");
      setModal(null);
      loadData();
    } catch(e) {
      showToast("Failed to save shift: " + (e.response?.data?.message || e.message));
    }
  };
  ...
  const handleDeleteShift = async (shiftId) => {
    try {
      ...
      showToast(t("shiftRemoved")||"Shift removed");
      setModal(null);
      loadData();
    } catch {
  ...
  const handlePublish = async () => {
    ...
    if (!window.confirm(`Publish ${draftCount} draft shifts? Employees will be notified.`)) return;
    setPublishing(true);
    try {
      const res = await api.post("/shifts/publish", { weekStart: isoDate(days[0]), weekEnd: isoDate(days[6]) });
      showToast(`🚀 Published ${res.data.count || draftCount} shifts`);
      loadData();
  ...
  const handleCopyPrevWeek = async () => {
    ...
    try {
      ...
      if (!prevShifts.length) { showToast(t("noPrevShifts")||"No shifts from last week"); return; }
      const calls = prevShifts.map(s => {
        const newDate = new Date(s.date);
        newDate.setDate(new Date(s.date).getDate() + 7);
      ...
      showToast(`✓ Copied ${calls.length} shifts from last week`);
      loadData();
    } catch { showToast("Failed to copy previous week"); }
  };
    ...
    <div className="su-page" style={{ paddingBottom:40 }}>
      ...
      {modal && (
        <ShiftModal
          ...
          onSave={handleSaveShift} onDelete={handleDeleteShift} onClose={() => setModal(null)}
        />
      )}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:14, marginBottom:20 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
          <button onClick={()=>setWeekOffset(o=>o-1)} style={{ background:"#f0f0ec", border:"none", borderRadius:8, padding:"8px 14px", cursor:"pointer", fontWeight:700, fontSize:13 }}>‹</button>
          <button onClick={()=>setWeekOffset(0)} style={{ background: weekOffset===0?"#f5b800":"#f0f0ec", border:"none", borderRadius:8, padding:"8px 14px", cursor:"pointer", fontWeight:700, fontSize:13 }}>Today</button>
          <span style={{ fontWeight:800, fontSize:14, color:"#2563eb", minWidth:190, textAlign:"center" }}>{weekLabel}</span>
          <button onClick={()=>setWeekOffset(o=>o+1)} style={{ background:"#f0f0ec", border:"none", borderRadius:8, padding:"8px 14px", cursor:"pointer", fontWeight:700, fontSize:13 }}>›</button>
          <button onClick={handleCopyPrevWeek} style={{ background:"#f0f0ec", border:"none", borderRadius:8, padding:"8px 14px", cursor:"pointer", fontWeight:600, fontSize:12, color:"#555" }}>
            📋 Copy Prev Week
          </button>
          <button onClick={handlePublish} disabled={publishing || draftCount===0}
            style={{ background: draftCount>0?"#22c55e":"#d1d5db", border:"none", borderRadius:10, padding:"9px 20px", cursor: draftCount>0?"pointer":"not-allowed", fontWeight:800, fontSize:13, color:"#fff" }}>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:18, flexWrap:"wrap" }}>
        {[
          ...
          <div key={s.label} style={{ background:"#1a1a1a", borderRadius:12, padding:"12px 18px", flex:1, minWidth:110 }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <button onClick={()=>setFilter("")} style={{ padding:"4px 12px", border:"1.5px solid", borderRadius:20, cursor:"pointer", fontSize:11, fontWeight:600, borderColor:!filter?"#1a1a1a":"#e0e0e0", background:!filter?"#1a1a1a":"#fff", color:!filter?"#fff":"#888" }}>All</button>
        {ROLES.map(r=>{
          const c = rc(r);
          return (
            <button key={r} onClick={()=>setFilter(filter===r?"":r)}
              style={{ padding:"4px 12px", border:"1.5px solid", borderRadius:20, cursor:"pointer", fontSize:11, fontWeight:600, borderColor:filter===r?c.bg:"#e0e0e0", background:filter===r?c.bg:"#fff", color:filter===r?"#fff":"#888" }}>{r}</button>
          );
        })}
      </div>

      {/* GRID */}
      <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 2px 20px rgba(0,0,0,.08)", border:"1px solid #f0f0ec" }}>

        {/* Header */}
        <div style={{ display:"grid", gridTemplateColumns:"190px repeat(7,1fr)", borderBottom:"2px solid #f0f0ec", position:"sticky", top:0, zIndex:10 }}>
          ...
          {days.map((d,i)=>{
            ...
            return (
              <div key={i} style={{ padding:"10px 8px", textAlign:"center", background: isToday?"#f5b800": i>=5?"#fafaf8":"#fff", borderRight: i<6?"1px solid #f0f0ec":"none" }}>
                <div style={{ fontSize:9, fontWeight:700, textTransform:"uppercase", letterSpacing:1, color: isToday?"#1a1a1a":"#aaa" }}>{t(DAY_T_KEYS[i])||DAY_T_KEYS[i]}</div>
                <div style={{ fontSize:22, fontWeight:900, color: isToday?"#1a1a1a":"#222", lineHeight:1.2 }}>{d.getDate()}</div>
                <div style={{ fontSize:9, color: isToday?"#1a1a1a99":"#bbb" }}>{d.toLocaleDateString("en",{month:"short"})}</div>
                {count > 0 && (
                  <div style={{ marginTop:3, background: isToday?"rgba(0,0,0,.15)":"#f0f0ec", borderRadius:20, padding:"1px 7px", display:"inline-block", fontSize:9, fontWeight:700, color: isToday?"#1a1a1a":"#888" }}>
                    {count}
                  </div>
                )}
              </div>
        ...
        {loading ? (
          ...
          <div style={{ padding:60, textAlign:"center", color:"#aaa" }}>
            ...
            <div>No employees found. Add employees via the <strong>Register Staff</strong> tab.</div>
          </div>
          ...
          return (
            <div key={empId} style={{ display:"grid", gridTemplateColumns:"190px repeat(7,1fr)", borderBottom: ei < visibleEmps.length-1 ? "1px solid #f0f0ec":"none" }}>
              <div style={{ padding:"10px 14px", borderRight:"1px solid #f0f0ec", background: ei%2===0?"#fff":"#fafaf8", display:"flex", flexDirection:"column", justifyContent:"center" }}>
                <div style={{ display:"flex", alignItems:"center", gap:9 }}>
                  <div style={{ width:32, height:32, borderRadius:"50%", background:"#f5b800", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:12, color:"#1a1a1a", flexShrink:0 }}>
                    {initials}
                  </div>
                  <div style={{ minWidth:0 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"#1a1a1a", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{empName}</div>
                    <div style={{ fontSize:10, color:"#aaa" }}>{emp.position}</div>
                  </div>
                </div>
                <div style={{ marginTop:6, fontSize:10, fontWeight:600, color: weekMins > 0 ? "#22c55e":"#ddd" }}>
              ...
              {days.map((d, di) => {
                ...
                return (
                  <div key={di}
                    ...
                    style={{ padding:"6px 5px", minHeight:80, borderRight: di<6?"1px solid #f0f0ec":"none", background: isToday?"#fffbeb": isWknd?"#f9f9f7": ei%2===0?"#fff":"#fafaf8", cursor: dayShifts.length ? "default":"pointer" }}
                  ...
                  >
                    {dayShifts.length === 0 ? (
                      <div style={{ height:"100%", display:"flex", alignItems:"center", justifyContent:"center", color:"#e0e0e0", fontSize:20, paddingTop:16, userSelect:"none" }}>+</div>
                    ) : (
                      dayShifts.map(s => {
                        const c    = rc(s.role);
                        ...
                        return (
                          <div key={s._id}
                            ...
                            style={{ background:c.bg, borderRadius:8, padding:"7px 8px", marginBottom:4, cursor:"pointer", boxShadow:`0 2px 6px ${c.bg}55`, border: s.isDraft ? "2px dashed rgba(255,255,255,.6)" : "2px solid transparent", position:"relative" }}>
                            {s.isDraft && (
                              <div style={{ position:"absolute", top:3, right:5, fontSize:8, color:"rgba(255,255,255,.8)", fontWeight:700, textTransform:"uppercase", letterSpacing:.5 }}>DRAFT</div>
                            )}
                            <div style={{ color:"#fff", fontSize:10, fontWeight:700, lineHeight:1.4 }}>{fmt12(s.startTime)}-{fmt12(s.endTime)}</div>
                            {mins > 0 && <div style={{ color:"rgba(255,255,255,.75)", fontSize:9 }}>{fmtHrs(mins)}</div>}
                            <div style={{ color:"rgba(255,255,255,.9)", fontSize:10, fontWeight:600, marginTop:2 }}>{s.role}</div>
                            {s.area && <div style={{ color:"rgba(255,255,255,.7)", fontSize:9 }}>📍 {s.area}</div>}
                          </div>
                        );
                      })
                    )}
                    {dayShifts.length > 0 && (
                      <button onClick={e=>{e.stopPropagation(); setModal({ emp, date: dateKey });}}
                        style={{ width:"100%", background:"none", border:"1px dashed #e0e0e0", borderRadius:6, color:"#ccc", fontSize:11, cursor:"pointer", padding:"3px 0", marginTop:2 }}>+ add</button>
                    )}
                  </div>
                );
              })}
        ...
        {!loading && visibleEmps.length > 0 && (
          <div style={{ display:"grid", gridTemplateColumns:"190px repeat(7,1fr)", borderTop:"2px solid #f0f0ec", background:"#f8f8f6" }}>
            ...
            {days.map((d, i) => {
              ...
              return (
                <div key={i} style={{ padding:"10px 8px", textAlign:"center", borderRight: i<6?"1px solid #f0f0ec":"none" }}>
                  <div style={{ fontSize:12, fontWeight:800, color: pct>=80?"#22c55e": pct>=50?"#f59e0b":"#dc2626" }}>{pct}%</div>
                  <div style={{ fontSize:9, color:"#bbb" }}>{count} · {fmtHrs(dayMins)}</div>
                </div>
              );
            })}
          </div>
